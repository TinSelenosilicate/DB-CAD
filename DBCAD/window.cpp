...
void Window::updateEntites(std::vector<int>& index) {
    for (auto id : index) {
        ENTITY_TREE_ITEM& eti = getEntityTreeItemByIndex(id);
        if (eti.ptrEntity && eti.isAutoUpdate) {
            std::vector<ENTITY*> e_base;
            for (auto i_b : eti.index_base) {
                ENTITY_TREE_ITEM& eti_b = getEntityTreeItemByIndex(i_b);
                if (eti_b.ptrEntity) {
...
ENTITY_TREE_ITEM& Window::getEntityTreeItemByIndex(int index) {
    for (auto& eti : entity_tree)
        if (eti.index == index) {
            return eti;
        }
